...
In its current stage, all that remains to do is to create an AI capable of playing the game. Currently, the AI only handles tricks with 1 card by selecting a random card of the correct suit to play. Ideally, it would be able to handle tricks with varying amounts of cards and play intelligently. All that remains is to write code that will mark the cards for the AI to play on a given trick - the code to actually play those cards, determine the winner, etc., has already been written. Since I will be taking a course in artificial intelligence next semester (Fall 2015), I may return to this project then.
